...
`renderer-olm-bundle` renders OLM registry+v1 bundles through
...
```bash
go get github.com/k8s-manifest-kit/renderer-olm-bundle
```
...
Registry TLS verification defaults to the renderer's current behavior (`false`); set
...
`Credentials: olmbundle.AmbientCredentials`; use `olmbundle.StaticCredentials` or a custom
